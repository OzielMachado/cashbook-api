...
router.get('/cashbook/cash', async (req, res, next)=>{
    auth= userController.verifyJWT(req.headers['x-access-token'])
    if(auth.idUser){
        if(req.headers.iduser==auth.idUser){
           resp= await movimentoController.cash();
           resp = Object.assign({}, resp, auth);
        }else{ 
            resp= {"status":"null", auth}
        }
...
router.get('/cashbook/io/:year/:month/:lastmonth', async (req, res, next)=>{
    auth= userController.verifyJWT(req.headers['x-access-token'])
    if(auth.idUser){
        if(req.headers.iduser==auth.idUser){
           resp= await movimentoController.anoMesMes(req.params);
        }else{ 
            resp= {"status":"null", auth}
        }
    }else{
...
router.get('/cashbook/:year/:month', async (req, res, next)=>{
    ...
    if(auth.idUser){
        if(req.headers.iduser==auth.idUser){
           resp= await movimentoController.mesAno(req.params);
        }else{ 
            resp= {"status":"null", auth}
        }
    }else{
